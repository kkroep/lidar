...
begin
  ...
  CO_out(0) <= ci_or_cyinit after 20 ps when S(0) = '1' else DI(0) when S(0) = '0' else 'X'; 
  CO_out(1) <= CO_out(0) after 20 ps when S(1) = '1' else DI(1) when S(1) = '0' else 'X'; 
  CO_out(2) <= CO_out(1) after 20 ps when S(2) = '1' else DI(2) when S(2) = '0' else 'X'; 
  CO_out(3) <= CO_out(2) after 20 ps when S(3) = '1' else DI(3) when S(3) = '0' else 'X'; 
